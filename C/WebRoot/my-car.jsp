...
function  load(pageNum) {
       $.ajax({
           ...
           data=$.parseJSON(data);//从数据库获得的json对象，已经包含查询回来的数据                       
			...
			+"<th class='tab-th-1'>"
			+"<a href='#'><img src='images/shangpinxiangqing/X1.png' width='100%' alt=''></a>"
			+"<input type='hidden' id='delproId' value="+news.product.id+"></input>"
			...
			+"<td><a href='#' id='option' onclick='delRow()'>删除</a></td>"
			+"</tr>"
			); 
			number+=1;
   ...
   function delRow(){ 
	   prodelId=$("#delproId").val();
	   	window.location.href="user?method=deleteCart&prodeId="+prodelId;
	    $("#option").parent().parent().remove();   
	}
   function pay(){
	   alert(numm1);
	   window.location.href="user?method=account&all="+numm1;
   }
</script>
<script type="text/javascript">
